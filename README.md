...
- ✅ **本机 IP 检测**: 自动检测当前 IP 的解锁状态(IPv4 和 IPv6)
- ✅ **IPv6 支持**: 所有流媒体服务完整支持 IPv6
- ✅ **代理支持**: 通过 SOCKS5/HTTP 代理测试(推荐)
- ✅ **X-Forwarded-For**: 备用方法(可能对某些服务无效)
...
**注意**: 菜单会根据您的网络配置自动调整:
- **双栈** (IPv4 + IPv6): 显示 6 个选项,包含 IPv4 和 IPv6 测试
- **仅 IPv4**: 显示 5 个选项,仅 IPv4 测试
- **仅 IPv6**: 显示 6 个选项,包含 IPv6 测试

### 选项 1: 检测本机 IP (IPv4)

自动检测当前公网 IPv4 地址并测试所有三个流媒体服务。
...
如果您的网络支持 IPv6,此选项会自动检测 IPv6 地址并测试。
...
通过代理服务器测试流媒体解锁状态。这是测试远程 IP 最准确的方法。
...
**示例:**
```
输入代理地址: socks5://192.168.1.100:1080
...
### 选项 4: 通过 X-Forwarded-For 检测

⚠️ **警告**: 此方法在 HTTP 请求中添加 `X-Forwarded-For` 头部,但大多数流媒体服务出于安全考虑会忽略此头部。检测结果可能不准确。
